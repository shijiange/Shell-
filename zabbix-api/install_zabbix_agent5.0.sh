...
#Author:            shijian
#Date:              2019-01-06
#FileName:          install_zabbix5_agent.sh
...
config_zabbix_agent (){ 
    sed -i -e "/^Server=127.0.0.1/c Server=$ZABBIX_SERVER"  -e "/^Hostname=Zabbix server/c Hostname=`hostname -I`"  /etc/zabbix/zabbix_agentd.conf
}
